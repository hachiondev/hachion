...
const[requestBatch,setRequestBatch]=useState([]);
  useEffect(() => {
    const fetchRequestBatch = async () => {
        try {
            const response = await axios.get('http://160.153.175.69:8080/requestbatch');
...
const handleDelete = async (batch_id) => {
  ...
  try { 
   const response = await axios.delete(`http://160.153.175.69:8080/requestbatch/delete/${batch_id}`); 
