...
export default function BatchTracking() {
  ...
  const handleSelectAll = (event) => {
    if (event.target.checked) {
      const allItemKeys = displayedData.map(item => `${item.student_id}-${item.batch_id}`);
  ...
  const handleSelectOne = (itemKey) => {
    if (selectedItems.includes(itemKey)) {
      ...
      if (newSelectedItems.length === displayedData.length) {
        setSelectAll(true);
      }
    }
  };
  ...
  useEffect(() => {
    const allCurrentPageKeys = displayedData.map(item => `${item.student_id}-${item.batch_id}`);
    const allSelected = allCurrentPageKeys.length > 0 &&
      allCurrentPageKeys.every(key => selectedItems.includes(key));
    setSelectAll(allSelected);
  }, [currentPage, displayedData, selectedItems]);

  // ADDED: Handle bulk delete
  const handleBulkDelete = async () => {
    if (selectedItems.length === 0) {
      setErrorMessage("❌ Please select at least one tracking record to delete");
      setSuccessMessage("");
      setTimeout(() => setErrorMessage(""), 3000);
      return;
    }

    const confirmMessage = `Are you sure you want to delete ${selectedItems.length} selected ${selectedItems.length === 1 ? 'tracking record' : 'tracking records'}?`;
    ...
    if (window.confirm(confirmMessage)) {
      try {
        ...
        const deletePromises = selectedItems.map(key => {
          ...
          return axios.delete(`https://api.test.hachion.co/studentsTracking/delete`, {
            params: {
              studentId: studentId,
              batchId: batchId
            }
        ...
        const updatedData = filteredData.filter(item =>
          !selectedItems.includes(`${item.student_id}-${item.batch_id}`)
        );

        setFilteredData(updatedData);
        setBatchTracking(updatedData);

        setSelectedItems([]);
        setSelectAll(false);

        setSuccessMessage(`✅ ${selectedItems.length} ${selectedItems.length === 1 ? 'tracking record' : 'tracking records'} deleted successfully`);
        setErrorMessage("");
        ...
        setSuccessMessage("");
        setErrorMessage("❌ Error deleting some tracking records. Please try again.");
        setTimeout(() => {
          setErrorMessage("");
        }, 6000);
      }
        ...
        <Table sx={{ minWidth: 700 }} aria-label="customized table">
            ...
            <TableRow>
              ...
              <StyledTableCell align='center' style={{ width: '30px' }}>
                <Checkbox
                  ...
                  indeterminate={selectedItems.length > 0 && selectedItems.length < displayedData.length}
                />
              </StyledTableCell>
          ...
          <TableBody>
            {displayedData.map((row, index) => {
              ...
              const itemKey = `${row.student_id}-${row.batch_id}`;
