 ...
 const [currentPage, setCurrentPage] = useState(1);
    ...
    useEffect(() => {
      const fetchResume = async () => {
          try {
              const response = await axios.get('http://160.153.175.69:8080/resume');
    ...
    const handleDeleteConfirmation = (resume_id) => {
      ...
      const handleSave = async () => {
        try {
            const response = await axios.put(
                `http://160.153.175.69:8080/videoaccess/update/${editedData.resume_id}`,editedData
            );
            setResume((prev) =>
                prev.map(curr =>
      ...
      const handleDelete = async (resume_id) => {
         ...
         try { 
          const response = await axios.delete(`http://160.153.175.69:8080/resume/delete/${resume_id}`); 
    ...
    const handleChange = (e) => {
      ...
      const handleSubmit = async (e) => {
        ...
        try {
          const response = await axios.post("http://160.153.175.69:8080/resume/add", dataToSubmit);
