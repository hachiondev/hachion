...
const AdminLogin = () => {
  ...
  const handleFormSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('https://api.test.hachion.co/api/v1/user/adminlogin', { email, password });
      ...
      <div className='login-left'>
        <div className='login-top'>
          ...
          <p className='go-to-register'>
            Don't have an account? <Link to='/adminregister' className='link-to-register'> Register </Link>
          </p>
        </div>
      </div>
      <LoginSide />
