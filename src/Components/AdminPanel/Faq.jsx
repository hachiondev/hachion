...
export default function Faq() {
  ...
  const handleSelectAll = (event) => {
    if (event.target.checked) {
      const allIds = displayedCategories.map(faq => faq.faq_id);
  ...
  useEffect(() => {
    const allCurrentPageIds = displayedCategories.map(faq => faq.faq_id);
  ...
  const handleBulkDelete = async () => {
    if (selectedIds.length === 0) {
      setErrorMessage("Please select at least one FAQ to delete");
      setTimeout(() => setErrorMessage(""), 3000);
      return;
    }

    const confirmMessage = `Are you sure you want to delete ${selectedIds.length} selected ${selectedIds.length === 1 ? 'FAQ' : 'FAQs'}?`;

    if (window.confirm(confirmMessage)) {
      try {
        // Delete all selected FAQs
        await axios.post("https://api.test.hachion.co/faq/delete", selectedIds);

        // Update state
        setAllData(prev => prev.filter(item => !selectedIds.includes(item.faq_id)));
        setFilteredCurriculum(prev => prev.filter(item => !selectedIds.includes(item.faq_id)));

        setSelectedIds([]);
        setSelectAll(false);

        setSuccessMessage(`${selectedIds.length} ${selectedIds.length === 1 ? 'FAQ' : 'FAQs'} deleted successfully`);
        setErrorMessage("");
        ...
        setSuccessMessage("");
        setErrorMessage("Error deleting some FAQs. Please try again.");
        setTimeout(() => {
          setErrorMessage("");
        }, 6000);
      }
  ...
  const handleDelete = async (faq_id) => {
    try {
      const response = await axios.delete(
        `https://api.test.hachion.co/faq/delete/${faq_id}`
      );
      ...
      setSelectedIds(prev => prev.filter(id => id !== faq_id));
                  ...
                  {rows.map((row, index) => (
                    ...
                    >
                      ...
                      <StyledTableCell align="center" sx={{ padding: 0 }}>
                        <GoPlus style={{ fontSize: '2rem', color: '#00AEEF', marginRight: '10px' }} onClick={addRow} />
                        <IoClose style={{ fontSize: '2rem', color: 'red' }} onClick={() => deleteRow(row.id)} />
                      </StyledTableCell>
                    </StyledTableRow>
                  ))}
                </TableBody>
                    ...
                    displayedCategories.map((course, index) => (
                      <StyledTableRow key={course.faq_id}>
                        <StyledTableCell align="center">
                          <Checkbox
                            checked={selectedIds.includes(course.faq_id)}
                            onChange={() => handleSelectOne(course.faq_id)}
                          />
                        </StyledTableCell>
                        <StyledTableCell align="center">
