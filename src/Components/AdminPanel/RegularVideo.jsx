  ...
  const [filterCourse,setFilterCourse]=useState([]);
    ...
    useEffect(() => {
      const fetchVideo = async () => {
          try {
              const response = await axios.get('https://api.test.hachion.co/regularvideo');
    ...
    const handleDeleteConfirmation = (regularvideo_id) => {
      ...
      const handleSave = async () => {
        try {
            const response = await axios.put(
                `https://api.test.hachion.co/regularvideo/update/${editedRow.regularvideo_id}`,
                editedRow
            );
      ...
      const handleDelete = async (regularvideo_id) => {
         ...
         try { 
          const response = await axios.delete(`https://api.test.hachion.co/regularvideo/delete/${regularvideo_id}`); 
    ...
    const handleChange = (e) => {
      ...
      const handleSubmit = async (e) => {
        ...
        try {
          const response = await axios.post("https://api.test.hachion.co/regularvideo/add", dataToSubmit);
...
<p style={{ marginBottom: 0 }}>Add Regular Video</p>
</div>
  ...
  <button className='reset-btn' onClick={handleReset}>Reset</button>
   ...
   <LocalizationProvider dateAdapter={AdapterDayjs}>
      <div className='course-category'>
        <div className='category'>
          <div className='category-header'>
            <p style={{ marginBottom: 0 }}>Regular Videos</p>
          </div>
