...
import DiscountCourseCard from '../HomePage/LimitedSection/components/DiscountCourseCard';
...
const DiscountCards = () => {
  const [currentPage, setCurrentPage] = useState(1); // Start at 1 for pagination logic
  ...
  const { data: courses } = useCoursesSummary()
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      if (width < 576) setCardsPerRow(1);
      else if (width < 992) setCardsPerRow(2);
  ...
  useEffect(() => {
    try {
      setDiscountRules(Array.isArray(discount) ? discount : []);
    } catch (e) {
  ...
  const startIndex = currentPage - 1; // Convert 1-based to 0-based
  ...
  return (
    <div className="position-relative text-center">
      ...
      <CardsPagination
        ...
        totalCards={orderedCourses.length}
        cardsPerPage={cardsPerRow}
