...
export default function CorporateCourses() {
  ...
  useEffect(() => {
    const filtered = trendingCourse.filter(
      (c) =>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://api.hachion.co/corporatecourse/add",
        courseData
      );
  ...
  const handleSave = async () => {
    try {
      const response = await axios.put(
        `https://api.hachion.co/corporatecourse/update/${editedData.corporatecourse_id}`,
        editedData
      );
      fetchCourses();
              ...
              <div className="category">
                ...
                <div className="course-details">
                  ...
                  <div className="course-row">
                    ...
                    <button className="reset-btn" onClick={resetCourseData}>
                      Reset
                    </button>
                  </div>
                </div>
                      ...
                      displayedCourse.map((row, index) => (
                        <StyledTableRow key={row.corporatecourse_id}>
                          ...
                          <StyledTableCell align="center">
                            ...
                            <RiDeleteBin6Line
                              onClick={() =>
                                handleDelete(row.corporatecourse_id)
                              }
                              className="delete"
                            />
                          </StyledTableCell>
        ...
        <DialogContent>
          <div className="col">
            ...
            <select
              ...
              onChange={(e) =>
                setEditedData({ ...editedData, category_name: e.target.value })
              }
            >
              <option value="">Select Category</option>
              {category.map((c) => (
          ...
          <div className="col" style={{ display: "flex", gap: 20 }}>
            ...
            <Switch
              ...
              onChange={(e) =>
                setEditedData({ ...editedData, status: e.target.checked })
              }
              color="primary"
            />
