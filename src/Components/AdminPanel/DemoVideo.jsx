...
export default function DemoVideo() {
  ...
  const [rows, setRows] = useState([
    { id: "", video_link: "", video_description: "", video_duration: "" },
  ]);
  ...
  const handleDelete = async (demovideo_id) => {
    try {
      const response = await axios.delete(
        `https://api.hachion.co/demovideo/delete/${demovideo_id}`
      );
  ...
  useEffect(() => {
    const filtered = demoVideo.filter(
      (demoVideo) =>
        demoVideo.course_name
          .toLowerCase()
          .includes(searchTerm.toLowerCase()) ||
        demoVideo.category_name
          .toLowerCase()
          .includes(searchTerm.toLowerCase()) ||
        demoVideo.video_description
          .toLowerCase()
          .includes(searchTerm.toLowerCase()) ||
        demoVideo.video_duration
          .toLowerCase()
          .includes(searchTerm.toLowerCase())
    );
    setFilteredVideo(filtered);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://api.hachion.co/demovideo/add",
        dataToSubmit
      );
                    ...
                    <StyledTableRow key={row.demovideo_id}>
                      ...
                      <StyledTableCell align="center">
                        ...
                        <RiDeleteBin6Line
                          className="delete"
                          onClick={() =>
                            handleDeleteConfirmation(row.demovideo_id)
                          }
                        />
                      </StyledTableCell>
                    </StyledTableRow>
