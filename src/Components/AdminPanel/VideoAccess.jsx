  ...
  const displayedCategories = filteredVideo.slice(
    ...
    useEffect(() => {
        const fetchTrainers = async () => {
          try {
            const response = await axios.get("http://160.153.175.69:8080/HachionUserDashboad/trainers");
    ...
    useEffect(() => {
      const fetchVideo = async () => {
          try {
              const response = await axios.get('http://160.153.175.69:8080/HachionUserDashboad/videoaccess');
    ...
    const handleDeleteConfirmation = (videoaccess_id) => {
      ...
      const handleSave = async () => {
        try {
            const response = await axios.put(
                `http://160.153.175.69:8080/HachionUserDashboad/videoaccess/update/${editedData.videoaccess_id}`,editedData
            );
            setVideoAccess((prev) =>
                prev.map(curr =>
      ...
      const handleDelete = async (videoaccess_id) => {
         ...
         try { 
          const response = await axios.delete(`http://160.153.175.69:8080/HachionUserDashboad/videoaccess/delete/${videoaccess_id}`); 
    ...
    const handleChange = (e) => {
      ...
      const handleSubmit = async (e) => {
        ...
        try {
          const response = await axios.post("http://160.153.175.69:8080/HachionUserDashboad/videoaccess/add", dataToSubmit);
