...
const CourseCategory = ({
  ...
  const formattedDate = courseData.date
    ? dayjs(courseData.date).format("MM-DD-YYYY")
    : null;
  ...
  const handleFilter = () => {
    ...
    if (startDate) {
      filteredData = filteredData.filter(
        (category) => new Date(category.date) >= new Date(startDate)
      );
    }
    if (endDate) {
      filteredData = filteredData.filter(
        (category) => new Date(category.date) <= new Date(endDate)
      );
    }
    if (searchTerm) {
  ...
  const handleEdit = async () => {
    try {
      ...
      setCategories((prev) =>
        prev.map((curr) => (curr.id === editedRow.id ? response.data : curr))
      );
  ...
  const handleDeleteConfirmation = (id) => {
    if (
      window.confirm("Are you sure you want to delete this Course Category?")
    ) {
      handleDelete(id);
    }
  };
  ...
  return (
    <>
      ...
      >
        ...
        >
          <Button onClick={handleEdit} className="update-btn">
            Update
          </Button>
        </DialogActions>
      </Dialog>
