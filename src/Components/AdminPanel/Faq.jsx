  ...
  const [searchTerm,setSearchTerm]=useState("")
    ...
    const handleDeleteConfirmation = (faq_id) => {
      ...
      const handleSave = async () => {
        try {
            const response = await axios.put(
                `http://160.153.175.69:8080/faq/update/${editedRow.faq_id}`,
                editedRow
            );
            setFaq((prev) =>
                prev.map(fa =>
      ...
      const handleDelete = async (faq_id) => {
         ...
         try { 
          const response = await axios.delete(`http://160.153.175.69:8080/faq/delete/${faq_id}`); 
    ...
    const handleChange = (e) => {
      ...
      const handleSubmit = async (e) => {
        ...
        try {
          const response = await axios.post("http://160.153.175.69:8080/faq/add", dataToSubmit);
