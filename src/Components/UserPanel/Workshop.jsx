    ...
    const fetchWorkshops = async () => {
      try {
        const response = await axios.get(
          "https://api.test.hachion.co/workshopschedule"
        );
        ...
        const categoriesUsed = Array.from(
          new Set(data.map((w) => w.category_name).filter(Boolean))
        );
        const coursesUsed = Array.from(
          new Set(data.map((w) => w.course_name).filter(Boolean))
        );
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Join hands-on IT workshops in the US! Get certified in high-demand tech skills with expert-led training. Boost your career today - limited seats available!"
        />
        <meta
          name="keywords"
          content="IT workshops USA, tech certification training, hands-on IT courses, best IT workshops 2025, IT skills training near me, certified tech workshops"
        />
        <meta
          property="og:title"
        ...
        <div className="workshop-page">
          <aside
            className={`workshop-sidebar ${
              showMobileFilters ? "show" : "hide"
            }`}
          >
            <h4>Filter</h4>
            <p className="workshop-note">
              Select either Category or Course filter not both.
            </p>
            <h6>Category</h6>
