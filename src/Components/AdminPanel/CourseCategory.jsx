...
const displayedCategories = filteredCategories.slice(
  ...
  const handleSubmit = async () => {
    try {
      const response = await axios.post("http://160.153.175.69:8080/HachionUserDashboad/course-categories/add", {
  ...
  const handleEdit = async () => {
    try {
        const response = await axios.put(
            `http://160.153.175.69:8080/HachionUserDashboad/course-categories/update/${editedRow.id}`,
            editedRow
        );
        setCategories((prev) =>
            prev.map(curr =>
  ...
  const handleDelete = async (id) => {
    ...
    try { 
     const response = await axios.delete(`http://160.153.175.69:8080/HachionUserDashboad/course-categories/delete/${id}`); 
