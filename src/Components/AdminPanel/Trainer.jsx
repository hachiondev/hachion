...
export default function Trainer() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      !trainerData.course_name ||
      !trainerData.category_name
    ) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setTrainers((prev) => [
          ...prev,
          { ...response.data, dateAdded: currentDate },
        ]);
        handleReset();
        setTimeout(() => {}, 5000);
      }
  ...
  const handleDateFilter = () => {
    const filtered = trainers.filter((trainer) => {
      ...
      return (
        (!startDate || trainerDate >= new Date(startDate)) &&
    ...
    try {
      ...
      setTrainers((prevTrainers) =>
        prevTrainers.map((trainer) =>
          trainer.trainer_id === selectedRow.trainer_id
            ? response.data
            : trainer
        )
      );
    ...
    <>
      {showAddCourse ? (
        <div className="course-category">
          <p>
            View Trainer <IoIosArrowForward /> Add Trainer{" "}
          </p>
                ...
                {trainers.map((row) => (
                  <StyledTableRow key={row.S_No}>
                    ...
                    <StyledTableCell align="center">
                      ...
                      <RiDeleteBin6Line
                        className="delete"
                        onClick={() => handleDeleteConfirmation(row.trainer_id)}
                      />
                    </StyledTableCell>
                  </StyledTableRow>
                ))}
