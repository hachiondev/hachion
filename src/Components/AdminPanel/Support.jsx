  ...
  const [searchTerm,setSearchTerm]=useState("")
    ...
    useEffect(() => {
      const fetchSupport = async () => {
          try {
              const response = await axios.get('http://160.153.175.69:8080/support');
    ...
    const handleDeleteConfirmation = (support_id) => {
      ...
      const handleSave = async () => {
        try {
            const response = await axios.put(
                `http://160.153.175.69:8080/support/update/${editedData.support_id}`,editedData
            );
            setSupport((prev) =>
                prev.map(curr =>
      ...
      const handleDelete = async (support_id) => {
         ...
         try { 
          const response = await axios.delete(`http://160.153.175.69:8080/support/delete/${support_id}`); 
      ...
      const handleSubmit = async (e) => {
        ...
        try {
          const response = await axios.post("http://160.153.175.69:8080/support/add", dataToSubmit);
