 ...
 const [currentPage, setCurrentPage] = useState(1);
    ...
    useEffect(() => {
      const fetchVideo = async () => {
          try {
              const response = await axios.get('https://api.test.hachion.co/demovideo');
    ...
    const handleDeleteConfirmation = (demovideo_id) => {
      ...
      const handleSave = async () => {
        try {
            const response = await axios.put(
                `https://api.test.hachion.co/demovideo/update/${editedRow.demovideo_id}`,
                editedRow
            );
            setDemoVideo((prev) =>
                prev.map(curr =>
      ...
      const handleDelete = async (demovideo_id) => {
         ...
         try { 
          const response = await axios.delete(`https://api.test.hachion.co/demovideo/delete/${demovideo_id}`); 
    ...
    const handleChange = (e) => {
      ...
      const handleSubmit = async (e) => {
        ...
        try {
          const response = await axios.post("https://api.test.hachion.co/demovideo/add", dataToSubmit);
