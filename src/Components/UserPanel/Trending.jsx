...
const Trending = () => {
  ...
  const categories = [
    ...
    { name: 'Project Management', cards: [
      { heading: 'Project Management', image: card2 }
    ]
  },
  { name: 'Business Intelligence', cards: [
      { heading: 'Python', image: card4 }
    ]
  },
  { name: 'Programming', cards: [
      { heading: 'Load Runner', image: card3 },
      { heading: 'QA Automation', image: card5 },
      { heading: 'QA Manual Testing', image: card8 }
    ]
  ...
  return (
    <>
      <div className='training-events'>
        ...
        <div className='courses-list'>
  {/* Display the first 7 course names */}
  ...
  {/* Display the remaining course names in the dropdown list */}
  <div className='dropdown-container'>
    ...
    {dropdownVisible && (
      <div className='dropdown-list'>
        {categories.slice(7).map(category => (
          <div 
            key={category.name} 
            className='dropdown-item' 
            onClick={() => handleDropdownSelection(category.name)}
          >
            {category.name}
          </div>
        ))}
      ...
      {filteredCards.slice(0, 4).map((card, index) => (
         ...
         {showMore && filteredCards.slice(4).map((card, index) => (
    <CourseCard
      key={index + 4}
      heading={card.heading}
      month='3 months'
  ...
  {filteredCards.length === 0 && <p>No courses available.</p>}
</div>
