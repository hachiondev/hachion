...
export default function StudentReview() {
  ...
  const handleApprove = async (review_id) => {
    try {
      ...
      const reviewToUpdate = review.find(
        (item) => item.review_id === review_id
      );
      ...
      const response = await axios.put(
        `https://api.hachion.co/userreview/update/${review_id}`,
        formData,
        {
  ...
  const handleReject = (review_id) => {
    ...
    setReview(updatedReviews);
    setFilteredReview(
      updatedReviews.filter((item) => item.review_id !== review_id)
    );
  };
  return (
    <>
