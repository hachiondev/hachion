...
export default function CandidateCertificate() {
  ...
  const handleGenerate = async () => {
    if (
      ...
      alert(
        "Please fill in all required fields before generating the certificate."
      );
      return;
    }
    ...
    try {
      const response = await fetch(
        "https://api.hachion.co/certificate/generate",
        {
          method: "POST",
          headers: {
  ...
  useEffect(() => {
    const fetchCourseNames = async () => {
      try {
        const response = await fetch(
          "https://api.hachion.co/enroll/coursenames"
        );
  ...
  useEffect(() => {
    if (certificateData.course_name) {
      const fetchStudents = async () => {
        try {
          const response = await fetch(
            `https://api.hachion.co/api/v1/user/students/${certificateData.course_name}`
          );
          const data = await response.json();
          setStudents(data);
        } catch (error) {
      ...
      if (certificateData.student_id) {
        try {
          const res = await fetch(
            `https://api.hachion.co/api/v1/user/lookup?studentId=${certificateData.student_id}`
          );
  ...
  useEffect(() => {
    const fetchByUserName = async () => {
      if (certificateData.student_name) {
        try {
          const res = await fetch(
            `https://api.hachion.co/api/v1/user/lookup?userName=${certificateData.student_name}`
          );
          const data = await res.json();
          setCertificateData((prev) => ({
            ...prev,
  ...
  useEffect(() => {
    const fetchCompletionDate = async () => {
      if (certificateData.course_name && certificateData.student_name) {
        try {
          const res = await fetch(
            "https://api.hachion.co/api/v1/user/completiondate",
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
  ...
  const handleSendEmail = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.hachion.co/certificate/send-email/${certificateId}`,
        {
          method: "POST",
        }
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `https://api.hachion.co/certificate/download/${certificateId}`
      );
              ...
              <div className="course-row">
                ...
                <div className="col">
                  ...
                  <input
                    ...
                    value={
                      certificateData.completed_date
                        ? dayjs(certificateData.completed_date).format(
                            "DD-MM-YYYY"
                          )
                        : ""
                    }
          ...
          />
            ...
            <div className="course-row">
              <button
                ...
                disabled={!certificateData.certificate_id}
              >
                Save Certificate
              </button>
              <button
                  ...
                  displayedCourse.map((curr, index) => (
                    <StyledTableRow key={curr.id}>
                      ...
                      <StyledTableCell align="center">
                        ...
                        curr.completionDate ? (
                          <a
                            href={`https://api.hachion.co/download/filename/${
                              curr.studentId
                            }_${curr.courseName.replaceAll(" ", "_")}_${dayjs(
                              curr.completionDate
                            ).format("YYYY_MM_DD")}_Certificate.pdf`}
                            download
                            target="_blank"
                      ...
                      <StyledTableCell align="center">
                        {dayjs(curr.completionDate).format("DD-MM-YYYY")}
                      </StyledTableCell>
                      <StyledTableCell align="center">
                        <div
                          style={{
