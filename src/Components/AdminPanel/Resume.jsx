 ...
 const [currentPage, setCurrentPage] = useState(1);
    ...
    useEffect(() => {
      const fetchResume = async () => {
          try {
              const response = await axios.get('/HachionUserDashboad/resume');
    ...
    const handleDeleteConfirmation = (resume_id) => {
      ...
      const handleSave = async () => {
        try {
            const response = await axios.put(
                `/HachionUserDashboad/videoaccess/update/${editedData.resume_id}`,editedData
            );
            setResume((prev) =>
                prev.map(curr =>
      ...
      const handleDelete = async (resume_id) => {
         ...
         try { 
          const response = await axios.delete(`/HachionUserDashboad/resume/delete/${resume_id}`); 
    ...
    const handleChange = (e) => {
      ...
      const handleSubmit = async (e) => {
        ...
        try {
          const response = await axios.post("/HachionUserDashboad/resume/add", dataToSubmit);
