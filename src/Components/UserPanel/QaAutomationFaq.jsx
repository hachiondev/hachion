    ...
    const fetchCourse = async () => {
      try {
        ...
        if (matchedCourse) {
          ...
          // Fetch faq details
          const faqResponse = await axios.get('http://localhost:8080/faq');
          ...
          if (matchedfaq && matchedfaq.faq_pdf) {
            const fullPdfUrl = `http://localhost:8080/faq/${matchedfaq.faq_pdf}`; // Ensure full URL
...
const downloadPdf = () => {
  ...
  if (faqWithPdf) {
    const pdfUrl = `http://localhost:8080/faq/${faqWithPdf.faq_pdf}`;
