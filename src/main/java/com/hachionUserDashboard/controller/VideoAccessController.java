...
public class VideoAccessController {
	...
	public ResponseEntity<VideoAccess> updateVideoAccess(@PathVariable int id,
			@RequestBody VideoAccess updatedVideoAccess) {
	...
	@DeleteMapping("/videoaccess/delete/{id}")
	public ResponseEntity<?> deleteVideoAccess(@PathVariable int id) {
		VideoAccess videoaccess = repo.findById(id).get();
		repo.delete(videoaccess);
		return null;
