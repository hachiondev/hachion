...
const PopupUpcomingCard = ({ mode, heading, month, date, time, duration, discountPercentage, image, trainer_name, level = "All Levels", scheduleCount }) => {
...
const handleShare = async (e) => {
  ...
  return (
    <div
      className="home-popup-card"
      onClick={isMobile ? navigateToCourse : undefined}
      role="button"
      tabIndex={0}
      onKeyDown={(e) => e.key === 'Enter' && isMobile && navigateToCourse()}
    >
      <div className="popup-card-action-icons">
            <button className="popup-card-icons" onClick={handleShare}><TbShare3 /></button>
            <button className="popup-card-icons" onClick={handleBookmark}>
                  {bookmarked ? <MdBookmark className="bookmark-active" /> : <MdBookmarkBorder />}
                </button>
          </div>
      <div className="card-header-div">
        <img src={image} alt="Course-img" className="home-popup-card-image" loading="lazy"
        onError={(e) => {
              e.target.onerror = null;
              e.target.src = fallbackImg;
            }}/>
        <div className="popup-course-card-header">
      ...
      <div className="card-course-details">
        <div className="card-row">
          <div className="popup-course-month">
           {month} Days
          </div>
          <div className={`popup-course-badge ${level?.toLowerCase()}`}>
            {level}
          </div>
          <div className="popup-trainer-name">
            By {trainer_name}
          </div>
        </div>
        <div className="card-row">
        <h3 className="popup-course-name">{heading}</h3>
        <div className="popup-discount-lable">
            {discountPercentage}% off
          </div>
        </div>
        <div className="popup-date-time-container">
          <p className="popup-card-date">{date}</p>
          <p className="popup-discount-duration">{time}</p>
        </div>

        <div className="more-schedules">
          <button
            className="popup-card-view-btn"
            onClick={(e) => {
              e.stopPropagation(); 
              navigateToCourse();
            }}
...
export default PopupUpcomingCard;