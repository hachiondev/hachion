  ...
  React.useEffect(() => {
    const fetchCertificates = async () => {
      ...
      try {
        ...
        const response = await fetch(`https://api.test.hachion.co/certificate/byname/${studentName}`);
              ...
              <TableBody>
                {certificates.map((certificate, index) => (
                  <TableRow key={certificate.certificateId}>
                    ...
                    <a
    href={`https://api.test.hachion.co/download/filename/${certificate.certificatePath.replace('certificates/', '')}`}
    target="_blank"
