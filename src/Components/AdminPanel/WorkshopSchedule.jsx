...
export default function WorkshopSchedule() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://api.test.hachion.co/workshopschedule/add",
        formDataToSend,
        {
          headers: {
  ...
  const handleSelectAll = (event) => {
    if (event.target.checked) {
      const allIds = displayedCategories.map(workshop => workshop.id);
  ...
  useEffect(() => {
    const allCurrentPageIds = displayedCategories.map(workshop => workshop.id);
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const response = await axios.delete(`https://api.test.hachion.co/workshopschedule/delete/${id}`);
      console.log("Workshop deleted successfully:", response.data);

      // Update state
      setCourses(prev => prev.filter(item => item.id !== id));
      setFilteredCourses(prev => prev.filter(item => item.id !== id));
      ...
      setErrorMessage("Failed to delete workshop.");
      setSuccessMessage("");
  ...
  const handleBulkDelete = async () => {
    if (selectedIds.length === 0) {
      setErrorMessage("Please select at least one workshop to delete");
      setTimeout(() => setErrorMessage(""), 3000);
      return;
    }

    const confirmMessage = `Are you sure you want to delete ${selectedIds.length} selected ${selectedIds.length === 1 ? 'workshop' : 'workshops'}?`;
    ...
    if (window.confirm(confirmMessage)) {
      try {
        ...
        await Promise.all(
          selectedIds.map(id =>
            axios.delete(`https://api.test.hachion.co/workshopschedule/delete/${id}`)
          )
        );
        ...
        setSuccessMessage(`${selectedIds.length} ${selectedIds.length === 1 ? 'workshop' : 'workshops'} deleted successfully`);
        setErrorMessage("");
        ...
        setSuccessMessage("");
        setErrorMessage("Error deleting some workshops. Please try again.");
        setTimeout(() => {
          setErrorMessage("");
        }, 6000);
      }
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await axios.put(
        `https://api.test.hachion.co/workshopschedule/update/${selectedRow.id}`,
        formDataToSend,
        {
          headers: {
            "Content-Type": "multipart/form-data",
                    ...
                    <StyledTableRow key={course.id}>
                      ...
                      <StyledTableCell align="center">
                        {course.banner_image ? <img
                          src={`https://api.test.hachion.co/${course.banner_image}`}
                          alt={`Banner`}
