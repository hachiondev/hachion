 ...
 const [currentPage, setCurrentPage] = useState(1);
    ...
    useEffect(() => {
      const fetchVideo = async () => {
          try {
              const response = await axios.get('http://160.153.175.69:8080/demovideo');
    ...
    const handleDeleteConfirmation = (demovideo_id) => {
      ...
      const handleSave = async () => {
        try {
            const response = await axios.put(
                `http://160.153.175.69:8080/demovideo/update/${editedRow.demovideo_id}`,
                editedRow
            );
            setDemoVideo((prev) =>
                prev.map(curr =>
      ...
      const handleDelete = async (demovideo_id) => {
         ...
         try { 
          const response = await axios.delete(`http://160.153.175.69:8080/demovideo/delete/${demovideo_id}`); 
    ...
    const handleChange = (e) => {
      ...
      const handleSubmit = async (e) => {
        ...
        try {
          const response = await axios.post("http://160.153.175.69:8080/demovideo/add", dataToSubmit);
