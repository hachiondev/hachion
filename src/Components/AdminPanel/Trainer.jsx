...
const[message,setMessage]=useState(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post("/HachionUserDashboad/trainer/add", trainerData
      );
...
const handleDelete = async (trainer_id) => {
   ...
   try { 
    const response = await axios.delete(`/HachionUserDashboad/trainer/delete/${trainer_id}`); 
...
const handleSave = async () => {
  ...
  try {
    const response = await axios.put(
      `/HachionUserDashboad/trainer/update/${selectedRow.trainer_id}`,
      editedRow
    );
