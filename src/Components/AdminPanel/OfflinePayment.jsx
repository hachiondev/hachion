...
export default function OfflinePayment() {
  ...
  const handleDeleteConfirmation = (id) => {
    if (
      window.confirm("Are you sure you want to delete this Offline payment")
    ) {
      handleDelete(id);
    }
  };
  const handleUpdate = async () => {
    try {
      const response = await axios.put(
        `https://api.hachion.co/offlinepayment/${paymentData.id}`,
        paymentData
      );
  ...
  const handleChange = (e) => {
    ...
    if (
      name === "installments" ||
      ["course_fee", "tax", "discount"].includes(name)
    ) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://api.hachion.co/offlinepayment/add",
        formData,
        {
          headers: {
