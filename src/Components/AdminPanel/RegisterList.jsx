...
const [currentPage, setCurrentPage] = useState(1);
    ...
    const handleDeleteConfirmation = (student_id) => {
      ...
      const handleDelete = async (student_id) => {
         ...
         try { 
          const response = await axios.delete(`https://api.test.hachion.co/registerstudent/delete/${student_id}`); 
    ...
    const handleUpdate = async () => {
      try {
        const response = await axios.put(
          `https://api.test.hachion.co/registerstudent/update/${studentData.student_id}`,
          studentData
        );
        setRegisterStudent((prev) =>
