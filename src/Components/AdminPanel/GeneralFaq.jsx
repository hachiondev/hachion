...
export default function GeneralFaq() {
  ...
  const handleSelectAll = (event) => {
    if (event.target.checked) {
      const allIds = displayedCategories.map(course => course.faqId);
  ...
  // Update selectedFaqIds when page changes or displayedCategories changes
  useEffect(() => {
    // Remove any selected IDs that are no longer on the current page
    const currentPageIds = displayedCategories.map(course => course.faqId);
    const updatedSelectedIds = selectedFaqIds.filter(id => currentPageIds.includes(id));
    if (updatedSelectedIds.length !== selectedFaqIds.length) {
      setSelectedFaqIds(updatedSelectedIds);
    }
  }, [currentPage, displayedCategories]);
  ...
  const handleDeleteConfirmation = (faqId) => {
    ...
    // If a specific faqId is provided (single delete), use that
    if (faqId) {
      idsToDelete = [faqId];
    }

    // If no faqId provided and nothing selected, show alert
    if (!faqId && (!idsToDelete || idsToDelete.length === 0)) {
      alert("Please select at least one FAQ to delete.");
      return;
    }

    const confirmMessage = faqId 
      ? "Are you sure you want to delete this FAQ?"
      : `Are you sure you want to delete ${idsToDelete.length} selected FAQ(s)?`;
  ...
  const handleDelete = async (ids) => {
    try {
      ...
      if (response.status === 200) {
        setSuccessMessage(`✅ ${ids.length} FAQ(s) deleted successfully.`);
        setErrorMessage("");
              ...
              <div className='entries'>
                ...
                <div className='entries-right'>
                  ...
                  {selectedFaqIds.length > 0 && (
                    ...
                    >
                      <RiDeleteBin6Line /> Delete Selected ({selectedFaqIds.length})
                    </button>
                  )}
          ...
          <Table sx={{ minWidth: 700 }} aria-label="customized table">
            <TableHead>
              <TableRow>
                ...
                <StyledTableCell align='center' sx={{ width: '100px' }}>
                  <Checkbox
                    checked={selectedFaqIds.length > 0 && selectedFaqIds.length === displayedCategories.length}
                    onChange={handleSelectAll}
                    indeterminate={selectedFaqIds.length > 0 && selectedFaqIds.length < displayedCategories.length}
                  />
                </StyledTableCell>
            ...
            <TableBody>
              {displayedCategories.length > 0 ? (
                ...
                <StyledTableRow>
                  <StyledTableCell colSpan={6} align="center">No FAQs found</StyledTableCell>
                </StyledTableRow>
              )}
            </TableBody>
          </Table>
