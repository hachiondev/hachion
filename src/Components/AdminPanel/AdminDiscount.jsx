...
const AdminDiscount = ({ onChange }) => {
  ...
  const handleSelectAll = (event) => {
    if (event.target.checked) {
      const allIds = displayedCoupon.map(coupon => coupon.discountId);
  ...
  const handleSelectOne = (discountId) => {
    if (selectedIds.includes(discountId)) {
      setSelectedIds(selectedIds.filter(selectedId => selectedId !== discountId));
      setSelectAll(false);
    } else {
      const newSelectedIds = [...selectedIds, discountId];
  ...
  useEffect(() => {
    const allCurrentPageIds = displayedCoupon.map(coupon => coupon.discountId);
  ...
  const handleBulkDelete = async () => {
    ...
    const confirmMessage = `Are you sure you want to delete ${selectedIds.length} selected ${selectedIds.length === 1 ? 'discount' : 'discounts'}?`;

    if (window.confirm(confirmMessage)) {
      try {
        // Delete all selected discounts
        const deletePromises = selectedIds.map(discountId =>
          axios.delete(`https://api.test.hachion.co/discounts-courses/${discountId}`)
        );

        await Promise.all(deletePromises);

        // Update state
        const updatedCoupon = coupon.filter(item => !selectedIds.includes(item.discountId));
        setCoupon(updatedCoupon);
        setAllCoupon(updatedCoupon);
        setFilteredCoupon(updatedCoupon);

        setSelectedIds([]);
        setSelectAll(false);

        setSuccessMessage(`✅ ${selectedIds.length} ${selectedIds.length === 1 ? 'discount' : 'discounts'} deleted successfully`);
        setErrorMessage("");
        ...
        setSuccessMessage("");
        setErrorMessage("❌ Error deleting some discounts. Please try again.");
        setTimeout(() => {
          setErrorMessage("");
        }, 6000);
      }
  ...
  const handleDelete = async (discountId) => {
    ...
    try {
      ...
      setSelectedIds(prev => prev.filter(selectedId => selectedId !== discountId));
                  ...
                  {displayedCoupon.length > 0 ? displayedCoupon.map((coupon, index) => (
                    <StyledTableRow key={coupon.discountId}>
                      ...
                      <StyledTableCell align="center">
                        <Checkbox
                          checked={selectedIds.includes(coupon.discountId)}
                          onChange={() => handleSelectOne(coupon.discountId)}
                        />
                      </StyledTableCell>
                    ...
                    <StyledTableRow>
                      ...
                      <StyledTableCell colSpan={13} align="center">No Discount Courses found</StyledTableCell>
                    </StyledTableRow>
                  )}
                </TableBody>
              </Table>
