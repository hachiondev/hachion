  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        const filteredData = response.data.filter(
          (course) =>
            course.schedule_course_name.toLowerCase().replace(/\s+/g, "-") ===
            courseName
        );
        setFilteredCourses(filteredData);
      } catch (error) {
        ...
        <div className="left">
          {filteredCourses.length > 0 ? (
            filteredCourses.map((course, index) => (
              <div
                className="partition"
                key={course.course_schedule_id || index}
              >
                <label className="radio-label">
                  <input
                    type="radio"
                    ...
                    checked={selectedBatch === course.course_schedule_id}
                    onChange={() =>
                      handleBatchSelect(course.course_schedule_id, course)
                    }
                    className="hidden-radio"
                  />
                  <span className="custom-radio"></span>
                  <div className="partition-schedule">
                    ...
                    <p
                      className={
                        course.schedule_mode === "Live Class" ? "class" : "demo"
                      }
                    >
                      <FaCircle
                        className={
                          course.schedule_mode === "Live Class"
                            ? "class-icon"
                            : "demo-icon"
                        }
                      />
