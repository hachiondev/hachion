...
const NavbarTop = () => {
  ...
  <div className="search-results" style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 10 }}>
    {searchResults.map(item => (
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <p style={{ fontWeight: 'bold' }}>{item.type === 'course' ? item.courseName : item.title}</p>
            <p style={{ color: '#555' }}>{item.type === 'course' ? item.courseCategory : item.category_name}</p>
          </div>
          <div>
          <img
  src={
    item.type === 'course'
      ? `/HachionUserDashboad/${item.courseImage}`
