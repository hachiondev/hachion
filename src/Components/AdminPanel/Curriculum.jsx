...
export default function Curriculum() {
  ...
  const handleDelete = async (curriculum_id) => {
    try {
      ...
      setSelectedIds(prev => prev.filter(id => id !== curriculum_id));
...
const matchesCourse =
  ...
  const handleSelectAll = (event) => {
    if (event.target.checked) {
      const allIds = displayedCategories.map(curriculum => curriculum.curriculum_id);
  ...
  useEffect(() => {
    const allCurrentPageIds = displayedCategories.map(curriculum => curriculum.curriculum_id);
  ...
  const handleBulkDelete = async () => {
    if (selectedIds.length === 0) {
      setErrorMessage("Please select at least one curriculum to delete");
      setTimeout(() => setErrorMessage(""), 3000);
      return;
    }

    const confirmMessage = `Are you sure you want to delete ${selectedIds.length} selected ${selectedIds.length === 1 ? 'curriculum' : 'curriculums'}?`;
    ...
    if (window.confirm(confirmMessage)) {
      try {
        ...
        await Promise.all(
          selectedIds.map(id =>
            axios.delete(`https://api.test.hachion.co/curriculum/delete/${id}`)
          )
        );

        // Update state
        setAllData(prev => prev.filter(item => !selectedIds.includes(item.curriculum_id)));
        setFilteredCurriculum(prev => prev.filter(item => !selectedIds.includes(item.curriculum_id)));

        setSelectedIds([]);
        setSelectAll(false);

        setSuccessMessage(`${selectedIds.length} ${selectedIds.length === 1 ? 'curriculum' : 'curriculums'} deleted successfully`);
        setErrorMessage("");
        ...
        console.error("Error deleting curriculums:", error);
        setSuccessMessage("");
        setErrorMessage("Error deleting some curriculums. Please try again.");
        setTimeout(() => {
          setErrorMessage("");
        }, 6000);
      }
      ...
      {showAddCourse ? (<div className='course-category'>
        ...
        <div>
          ...
          {(filterData.category_name || filterData.course_name) ? (
                ...
                <TableBody>
                  {displayedCategories.length > 0 ? (
                    displayedCategories.map((course, index) => (
                      <StyledTableRow key={course.curriculum_id}>
                        <StyledTableCell align="center">
                          <Checkbox
                            checked={selectedIds.includes(course.curriculum_id)}
                            onChange={() => handleSelectOne(course.curriculum_id)}
                          />
                        </StyledTableCell>
                        <StyledTableCell align="center">
            ...
            <p>Please select category and courses to display data</p>
          )}
