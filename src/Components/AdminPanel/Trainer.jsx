...
//   const isEditing = Boolean(trainerData.trainer_id);
...
//           `https://api.test.hachion.co/trainer/update/${trainerData.trainer_id}`,
//           toBackendTrainer(trainerData)
//         );
//         const updated = normalizeTrainer(response.data);
//         setTrainers((prev) =>
//           prev.map((t) => (t.trainer_id === trainerData.trainer_id ? updated : t))
//         );
...
//       const start = startDate ? new Date(startDate).setHours(0, 0, 0, 0) : null;
//       const end = endDate ? new Date(endDate).setHours(23, 59, 59, 999) : null;
//       return (!start || trainerDate >= start) && (!end || trainerDate <= end);
//     });
//     setFilteredTrainers(filtered);
//   };
...
//         `https://api.test.hachion.co/trainer/delete/${trainer_id}`
//       );
//       if (response.status === 200) {
//         setSuccessMessage("✅ Trainer deleted successfully.");
//         setTrainers((prev) => prev.filter((trainer) => trainer.trainer_id !== trainer_id));
...
//   useEffect(() => {
//     axios
//       .get("https://api.test.hachion.co/course-categories/all")
//       .then((response) => {
//         setCourse(response.data);
//         setCategories(response.data);
//       })
...
//       .then((response) => setCourseCategory(response.data))
...
//     if (trainerData.category_name) {
//       const filtered = courseCategory.filter(
//         (c) => c.courseCategory === trainerData.category_name
//       );
//       setFilterCourse(filtered);
//     } else {
//       setFilterCourse([]);
...
//   const handleAddTrendingCourseClick = () => setShowAddCourse(true); 
//   const handleClickOpen = (row) => { console.log("Edit row data:", row); 
//     setSelectedRow(row); 
//     setTrainerData(row)  
//     console.log("tid",row.trainer_id) 
//   };
...
//                     value={trainerData.summary}
//                     onChange={(val) =>
//                       setTrainerData((prev) => ({ ...prev, summary: val }))
//                     }
...
//                     Filter
//                   </button>
//                   <button className="filter" onClick={handleDateReset}>
//                     Reset
//                   </button>
//                 </div>
//                 <div className="entries">
...
//                       }}
//                     >
//                       <FiPlus className="icon" /> Add Trainer
//                     </button>
//                   </div>
//                 </div>
...
//                           <StyledTableRow key={row.id}> 
//                           <StyledTableCell align="center"><Checkbox /></StyledTableCell> 
//                           <StyledTableCell align="center">{index + 1 + (currentPage - 1) * rowsPerPage}</StyledTableCell> <StyledTableCell align="center"> {row.profileImagee ? <img src={row.profileImage} alt="User" width="50" height="50" /> : 'No Image'} </StyledTableCell> <StyledTableCell align="left">{row.trainer_name}</StyledTableCell> 
//                           <StyledTableCell align="center">{row.trainerRating}</StyledTableCell> 
//                           <StyledTableCell sx={{ width: 100 }} align="left">{row.course_name}</StyledTableCell> 
//                           <StyledTableCell sx={{ width: 200, whiteSpace: 'pre-wrap' }} align="left">{row.demo_link_1}</StyledTableCell> 
//                           <StyledTableCell sx={{ width: 200, whiteSpace: 'pre-wrap' }} align="left">{row.demo_link_2}</StyledTableCell> 
//                           <StyledTableCell sx={{ width: 200, whiteSpace: 'pre-wrap' }} align="left">{row.demo_link_3}</StyledTableCell> 
//                           <StyledTableCell sx={{ width: 400, whiteSpace: 'pre-wrap' }} align="left">{row.summary}</StyledTableCell> 
//                           <StyledTableCell align="center">{row.date}</StyledTableCell> <StyledTableCell align="center"> 
...
//                           {errorMessage && ( <p style={{ color: "red", fontWeight: "bold", margin: 0 }}>{errorMessage}</p> )} {message && <p style={{ color: 'green' }}>
//                 Table updated successfully</p>}
